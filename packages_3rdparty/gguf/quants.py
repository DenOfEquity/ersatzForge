...
class __Quant(ABC):
    ...
    def init_grid(cls):
        ...
        grid = grid.reshape((-1, 1)) >> np.array(list(range(0, 8, 8 // elems_per_byte)), dtype=np.uint8).reshape((1, elems_per_byte))
...
class Q3_K(__Quant, qtype=GGMLQuantizationType.Q3_K):
    ...
    def dequantize_blocks(cls, blocks: np.ndarray) -> np.ndarray:
        ...
        qh = hmask.reshape(n_blocks, -1, 1, 32) >> np.array([0, 1, 2, 3, 4, 5, 6, 7], dtype=np.uint8).reshape((1, 1, 8, 1))
    ...
    def dequantize_blocks_pytorch(cls, blocks, block_size, type_size, parameter) -> torch.Tensor:
        ...
        qh = hmask.reshape(n_blocks, -1, 1, 32) >> torch.tensor([0, 1, 2, 3, 4, 5, 6, 7], device=d.device, dtype=torch.uint8).reshape((1, 1, 8, 1))
...
class Q5_K(__Quant, qtype=GGMLQuantizationType.Q5_K):
    ...
    def dequantize_blocks(cls, blocks: np.ndarray) -> np.ndarray:
        ...
        qh = qh.reshape((n_blocks, -1, 1, 32)) >> np.array([0, 1, 2, 3, 4, 5, 6, 7], dtype=np.uint8).reshape((1, 1, 8, 1))
    ...
    def dequantize_blocks_pytorch(cls, blocks, block_size, type_size, parameter) -> torch.Tensor:
        ...
        qh = qh.reshape((n_blocks, -1, 1, 32)) >> torch.tensor([0, 1, 2, 3, 4, 5, 6, 7], device=d.device, dtype=torch.uint8).reshape((1, 1, 8, 1))
...
class IQ2_XXS(__Quant, qtype=GGMLQuantizationType.IQ2_XXS):
    ...
    def dequantize_blocks(cls, blocks: np.ndarray) -> np.ndarray:
        ...
        signs = signs.reshape((n_blocks, -1, 4, 1)) >> np.array([0, 1, 2, 3, 4, 5, 6, 7], dtype=np.uint8).reshape((1, 1, 1, 8))
...
class IQ2_XS(__Quant, qtype=GGMLQuantizationType.IQ2_XS):
    ...
    def dequantize_blocks(cls, blocks: np.ndarray) -> np.ndarray:
        ...
        signs = signs.reshape((n_blocks, -1, 1)) >> np.array([0, 1, 2, 3, 4, 5, 6, 7], dtype=np.uint8).reshape((1, 1, 8))
        signs = signs & np.uint8(0x01)
        signs = np.where(signs == 0, np.float32(1), np.float32(-1))
...
class IQ3_S(__Quant, qtype=GGMLQuantizationType.IQ3_S):
    ...
    def dequantize_blocks(cls, blocks: np.ndarray) -> np.ndarray:
        ...
        qh = qh.reshape((n_blocks, -1, 1)) >> np.array([0, 1, 2, 3, 4, 5, 6, 7], dtype=np.uint8)
...
class IQ4_XS(__Quant, qtype=GGMLQuantizationType.IQ4_XS):
    ...
    def dequantize_blocks(cls, blocks: np.ndarray) -> np.ndarray:
        ...
        scales_h = scales_h.reshape((n_blocks, 1, -1)) >> np.array([0, 2, 4, 6, 8, 10, 12, 14], dtype=np.uint16).reshape((1, -1, 1))
