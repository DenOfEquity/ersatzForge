...
class ersatzZImageTurboControl(scripts.Script):
    ...
    def ui(self, *args, **kwargs):
        with InputAccordion(False, label=self.title()) as enabled:
            ...
            with gradio.Row():
                ...
                with gradio.Column():
                    image_str = gradio.Slider(value=0.8, minimum=0.0, maximum=2.0, step=0.01, label="strength")
                    image_stop = gradio.Slider(value=0.85, minimum=0.0, maximum=1.0, step=0.01, label="stop sigma")
                    with gradio.Row():
    ...
    def process_before_every_sampling(self, params, *script_args, **kwargs):
        ...
        if enabled and image is not None and strength > 0.0 and stop > 0.0 and params.sd_model.is_lumina2 and getattr(shared.sd_model.forge_objects.unet.model.diffusion_model, "control", False):
            ...
            extra_mem = n * k_latent.shape[1] * k_latent.shape[2] * k_latent.shape[3] * x.element_size() * 1024
