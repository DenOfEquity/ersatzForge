...
# from local_sox.transform import Transformer as SoxTransformer
import copy
import torch
import operator
import onnxruntime
...
class XVectorExtractor(nn.Module):
    ...
    def normalize_db(waveform: torch.Tensor, target_db: float) -> torch.Tensor:
        if waveform.numel() == 0:
            return waveform
        ...
        if peak == 0:
            return waveform  # Avoid division by zero for silent audio
    ...
    def extract_code(self, audio):
        with torch.no_grad():
            ...
            norm_audio = torch.from_numpy(copy.deepcopy(audio)).unsqueeze(0)
            norm_audio = normalize_db(norm_audio, -6.0)
