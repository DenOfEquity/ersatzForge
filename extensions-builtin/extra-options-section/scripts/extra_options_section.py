...
class ExtraOptionsSection(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        if is_img2img:
            extra_options = shared.opts.extra_options_img2img
            elem_id_tabname = "extra_options_img2img"
        else:
            extra_options = shared.opts.extra_options_txt2img
            elem_id_tabname = "extra_options_txt2img"
        ...
        with gr.Blocks() as interface:
            with gr.Group(elem_id=elem_id_tabname):
                ...
                columns = shared.opts.extra_options_cols
                row_count = int((len(extra_options) + (columns - 1))/ columns)
        ...
        interface.load(fn=get_settings_values, inputs=None, outputs=self.comps, queue=False, show_progress=False)
...
"""),
    ...
    "extra_options_cols": shared.OptionInfo(1, "Number of columns for added settings", gr.Slider, {"step": 1, "minimum": 1, "maximum": 4}).info("displayed amount will depend on the actual browser window width").needs_reload_ui(),
}))
