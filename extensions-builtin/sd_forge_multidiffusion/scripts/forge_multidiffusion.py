...
class MultiDiffusionForForge(scripts.Script):
    ...
    def ui(self, *args, **kwargs):
        with InputAccordion(False, label=self.title()) as enabled:
            ...
            with gr.Row():
                ...
                detect_size = ToolButton(value='\U0001F4D0', elem_id=self.elem_id("detect_size"), tooltip="Auto detect size from image")
            with gr.Row():
        ...
        detect_size.click(
            fn=lambda w, h: (64*((32+w)//64) or gr.skip(), 64*((32+h)//64) or gr.skip()),
    ...
    def process(self, p, enabled: bool, method: str, tile_width: int, tile_height: int, tile_overlap: int, tile_batch_size: int, **kwargs):
        if enabled:
