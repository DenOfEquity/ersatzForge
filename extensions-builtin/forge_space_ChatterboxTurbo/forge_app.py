...
def generate(
    ...
    print (f"[ChatterboxTurbo] Done. (Seed: {seed_num})")

    return (MODEL.sr, wav.squeeze(0).cpu().numpy())


with gr.Blocks(analytics_enabled=False, css=CUSTOM_CSS, title="Chatterbox Turbo") as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Accordion("Advanced Options", open=False):
                seed_num = gr.Number(value=-1, label="Seed (-1 for random)")
