...
class ApplyPulid:
    def apply_pulid(self, model, pulid, eva_clip, face_analysis, image, weight, start_at, end_at, sharpening=0.0, method=None, noise=0.0, fidelity=8, attn_mask=None):
        ...
        if fidelity > 0:
            zero_tensor = torch.zeros((1, fidelity, cond.size(-1)), dtype=dtype, device=device)
            cond = torch.cat([cond, zero_tensor], dim=1)
            if noise > 0:
                zero_tensor = torch.randn((1, fidelity, cond.size(-1)), dtype=dtype, device=device) * noise
