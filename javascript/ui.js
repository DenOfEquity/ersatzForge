...
function showSubmitButtons(tabname, show) {
    ...
    if (show) {
        if ((opts.generate_forever_t2i && tabname === "txt2img") || (opts.generate_forever_i2i && tabname === "img2img")) {
            ...
            submit.click(); // add a Setting for a delay?
...
function currentImg2imgSourceResolution(w, h, r, s) {
    var img = gradioApp().querySelector('#mode_img2img > div[style="display: block;"] :is(img, canvas)');
    return img ? [img.naturalWidth || img.width, img.naturalHeight || img.height, r, s] : [0, 0, r, s];
}
