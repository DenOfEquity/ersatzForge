...
class Lumina2(ForgeDiffusionEngine):
    ...
    def __init__(self, estimated_config, huggingface_components):
        ...
        k_predictor = PredictionDiscreteFlow(shift=6.0, multiplier=1.0)
    ...
    def get_prompt_lengths_on_ui(self, prompt):
        token_count = self.text_processing_engine_gemma.tokenize_for_UI(prompt)
...
class Zimage(ForgeDiffusionEngine):
    ...
    @torch.inference_mode()
    def get_prompt_lengths_on_ui(self, prompt):
        token_count = self.text_processing_engine_qwen.tokenize_for_UI(prompt)
        return token_count, -1
