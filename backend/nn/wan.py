...
class WanModel(nn.Module):
    ...
    def __init__(
        ...
        in_channels=16, # redundant addition to help model id
    ):
        r"""
    ...
    def forward(self, x, timestep, context, clip_fea=None, time_dim_concat=None, transformer_options={}, **kwargs):
        ...
        pad_h = (patch_size[1] - (x.shape[-2] % patch_size[1])) % patch_size[1]
        pad_w = (patch_size[2] - (x.shape[-1] % patch_size[2])) % patch_size[2]
        ...
        if time_dim_concat is not None:
            pad_h = (patch_size[1] - (time_dim_concat.shape[-2] % patch_size[1])) % patch_size[1]
            pad_w = (patch_size[2] - (time_dim_concat.shape[-1] % patch_size[2])) % patch_size[2]
