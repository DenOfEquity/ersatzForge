...
class Script(scripts.Script):
    ...
    def run(self, p, put_at_start, different_seeds, prompt_type, variations_delimiter, margin_size):
        ...
        last_part_index = len(prompt_matrix_parts) - 1

        combination_count = 2 ** last_part_index
        ...
        p.n_iter = prompt_count
        p.batch_size = 1 # cond padding will change results

        p.do_not_save_grid = True

        print(f"[Prompt matrix] will generate {p.n_iter} images.")

        if prompt_type == "positive":
            p.prompt = all_prompts
        else:
            p.negative_prompt = all_prompts
        p.seed = [p.seed + (i if different_seeds else 0) for i in range(prompt_count)]
        processed = process_images(p)

        if prompt_count > 1:
            grid = images.image_grid(processed.images, p.batch_size, rows=1 << (last_part_index // 2))
