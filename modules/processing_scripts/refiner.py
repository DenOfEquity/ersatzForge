...
class ScriptRefiner(scripts.ScriptBuiltinUI):
    ...
    def ui(self, is_img2img):
        choices = ([""] if is_img2img else ["", "[STOP]"]) + sd_models.checkpoint_tiles()

        with InputAccordion(False, label="Refiner", elem_id=self.elem_id("enable")) as enable_refiner:
            with gr.Row():
                refiner_checkpoint = gr.Dropdown(show_label=False, label="Refiner", info="(use model of same architecture)", elem_id=self.elem_id("checkpoint"), choices=choices, value="", tooltip="switch to another model in the middle of generation")
        ...
        self.infotext_fields = [
            ...
            PasteField(refiner_checkpoint, "Refiner", api="refiner_checkpoint"),
