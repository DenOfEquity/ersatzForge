...
class ScriptRefiner(scripts.ScriptBuiltinUI):
    ...
    def ui(self, is_img2img):
        choices = [""] if is_img2img else ["", "[STOP]"]

        with InputAccordion(False, label="Refiner", elem_id=self.elem_id("enable")) as enable_refiner:
            with gr.Row():
                refiner_checkpoint = gr.Dropdown(label="Checkpoint", info="(use model of same architecture)", elem_id=self.elem_id("checkpoint"), choices=choices + sd_models.checkpoint_tiles(), value="", tooltip="switch to another model in the middle of generation")
                create_refresh_button(refiner_checkpoint, sd_models.list_models, lambda: {"choices": choices + sd_models.checkpoint_tiles()}, self.elem_id("checkpoint_refresh"))
    ...
    def setup(self, p, enable_refiner, refiner_checkpoint, refiner_switch_at):
        ...
        if not enable_refiner or refiner_checkpoint in (None, ""):
